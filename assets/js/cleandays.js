...
(function(){
  ...
  function reloadWith(next){
    ...
    location.href = `cleandays.html?${q.toString()}`;
  }
  ...
  // 제목 오른쪽 버튼 → 3-2 페이지 이동
(function(){
  const btn = document.querySelector('#goDateReco');
  if(!btn) return;
  ...
  // -------------------------------
  // 3) 데이터 로드
  //    API: GET /api/air-quality/best-period?region=&start=&end=
